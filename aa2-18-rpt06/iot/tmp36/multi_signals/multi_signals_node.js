...
parser.on("data", (data) => {
  ...
  secondcommand = readData.indexOf(",", firstcommaidx + 1);
  if (firstcommaidx > 0) {
    
    lux = readData.substring(firstcommaidx + 1);
    humi - readData.substring(firstcommaidx + 1, secondcommand);
    temp = readData.substring(secondcommand + 1);
    readData = "";
